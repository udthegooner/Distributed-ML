...
def train_model(model, train_loader, optimizer, criterion, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        for i, param in enumerate(model.parameters()):
            tensor_list = param.grad
            dist.all_reduce(tensor_list)

            # print("PARAMETER GRADIENTS BEFORE MEAN", param.grad)
            mean = tensor_list / dist.get_world_size()
            param.grad = mean
            # print("PARAMETER GRADIENTS AFTER MEAN", param.grad)
